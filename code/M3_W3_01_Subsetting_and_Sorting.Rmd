...
The same can be done with the plyr package and the arrange command. The arrange command takes as arguments 
a dataframe and a variable. The default is ascending order, but of course there is a command for descending order
...
This is quite straightforward. In the example below a vector (in this case a series of random normals) is assigned
to a new column (var4)
...
Another way you can do this is by using the cbind command. In the example below the vector rnorm(5)
is bound as a column to the right side of X. Inverting the order of the dataframe and the vector will bind
the column to the left side of the dataframe
...
Rows can be added by using the rbind command. Analogously, if place the vector to the right of the dataframe
rbind will bind the vector to the bottom of the dataframe, and if you put it to the left it will bind it
to the top of the dataframe
